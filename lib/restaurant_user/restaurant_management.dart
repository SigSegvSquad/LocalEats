...
class RestaurantManagement {
  ...
  Future<void> deleteData(restaurantData, restUser) async {
    var foodName = restaurantData['foodName'];
    //var price = restaurantData['price'];
    //var imageUrl = restaurantData['imageUrl'];
    if (isLoggedIn()) {
      Firestore.instance
          ...
          .document(restUser.uid)
          .updateData({
        "menu.${foodName}.imgae_url": FieldValue.delete(),
        "menu.${foodName}.name": FieldValue.delete(),
        "menu.${foodName}.price": FieldValue.delete(),
      }).catchError((e) {
        print(e);
      });
    } else {
